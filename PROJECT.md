...
| 项 | 值 |
| --- | --- |
| 当前版本号 | `dev`（未发布正式 tag；`version.json` 中约定为 `dev`） |
| 最近一次提交 | `d98691c` feat: 升级为完整 PWA |
...
| ⚠️ | 版本自动检测与「发现新版本」弹窗逻辑已在本地部分实现（`version.json` + `index.html` 内联脚本 + `icon-1024`），但**尚未单独提交**（见 TODO P1） |
...
1. **v1.0（稳定版）**：补全文档体系 + 版本自动检测提交 + 数据契约冻结。
